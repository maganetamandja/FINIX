#include "Technetium.hpp"

    #include <iostream>
    #include <vector>
    #include "../elementos_basicos.hpp"
    using namespace std;

    /*struct atom {
        string atom_name;
        vector<bool_atomic> my_atom_bool_v;
        vector<integer_atomic> my_atom_int_v;
        vector<string_atomic> my_atom_str_v;
        vector<double_atomic> my_atom_d_v;
    };*/
    
atom Technetium ;
void create_Technetium(){
Technetium.atom_name = "Technetium" ;
string_atomic proprety ;
proprety.set_string_atomic("Name", "Technetium");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Symbol", "Tc");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Atomic_Number", "43");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Atomic_Weight", "98");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Density", "11500");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Melting_Point", "2430.15");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Boiling_Point", "4538.15");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Phase", "Solid");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Absolute_Melting_Point", "2430");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Absolute_Boiling_Point", "4538");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Critical_Pressure", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Critical_Temperature", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Heat_of_Fusion", "23");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Heat_of_Vaporization", "550");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Heat_of_Combustion", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Specific_Heat", "182");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Adiabatic_Index", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Neel_Point", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Thermal_Conductivity", "51");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Thermal_Expansion", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Density_Liquid", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Molar_Volume", "0.0000090");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Brinell_Hardness", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Mohs_Hardness", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Vickers_Hardness", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Bulk_Modulus", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Shear_Modulus", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Young_Modulus", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Poisson_Ratio", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Refractive_Index", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Speed_of_Sound", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Valence", "7");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Electronegativity", "1.9");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("ElectronAffinity", "53");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Autoignition_Point", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Flashpoint", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("DOT_Hazard_Class", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("DOT_Numbers", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("EU_Number", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("NFPA_Fire_Rating", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("NFPA_Health_Rating", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("NFPA_Reactivity_Rating", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("RTECS_Number", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Alternate_Names", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Block", "d");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Group", "7");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Period", "5");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Electron_Configuration", "[Xe]4f76s2");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Color", "Silver");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Gas_phase", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("CAS_Number", "CAS7440-26-8");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("CID_Number", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Gmelin_Number", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("NSC_Number", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Electrical_Type", "Conductor");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Electrical_Conductivity", "5000000");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Resistivity", "0");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Superconducting_Point", "7.8");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Magnetic_Type", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Curie_Point", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Mass_Magnetic_Susceptibility", "0.000000");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Molar_Magnetic_Susceptibility", "0.0000000");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Volume_Magnetic_Susceptibility", "0.0003930");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Percent_in_Universe", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Percent_in_Sun", "0");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Percent_in_Meteorites", "0");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Percent_in_Earths_Crust", "0");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Percent_in_Oceans", "0");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Percent_in_Humans", "0");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Atomic_Radius", "0.000000");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Covalent_Radius", "0.000000");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Van_der_Waals_Radius", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Space_Group_Name", "P63/mmc");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Space_Group_Number", "194");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("HalfLife", "129944800000000.0000000000");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Lifetime", "189870800000000.0000000000");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Decay_Mode", "BetaDecay");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Quantum_Numbers", "6S5/2");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Neutron_Cross_Section", "22");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Neutron_Mass_Absorption", "");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Graph.Period", "5");
Technetium.my_atom_str_v.push_back(proprety) ;
proprety.set_string_atomic("Graph.Group", "7");
Technetium.my_atom_str_v.push_back(proprety) ;
}
